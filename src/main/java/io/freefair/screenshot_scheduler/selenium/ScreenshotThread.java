...
public class ScreenshotThread implements Runnable {
	...
	public void run() {
		try {
			...
			((RemoteWebDriver)scheduledSeleniumSession.getSession().getDriver()).executeScript("document.body.style.zoom = '" + zoomInPercent + "%'; window.scrollTo(0, " + scheduledSeleniumSession.getYScroll() + ");");
